...
@nb.njit(cache=True, fastmath=True)  # type: ignore
...
@nb.njit(fastmath=True, cache=True) # type: ignore
...
@nb.njit(cache=True, fastmath=True) # type: ignore
def estimate_norm_batch(lmks, image_size=112):
    Ms = []
    for lmk in lmks:
        Ms.append(estimate_norm(lmk, image_size))
    return Ms
...
def norm_crop_batched(img, landmarks, image_size=112):
    Ms = estimate_norm_batch(landmarks, image_size)
    crops = []
    for M in Ms:
        warped = cv2.warpAffine(img, M, (image_size, image_size), borderValue=0.0)
        crops.append(warped)
