...
class GRIDDataset(Dataset):
    def __init__(self,dataset_cfg,transform_cfg,mode):
        ...
        self.env = lmdb.open(os.path.join(self.dataset_cfg.get('data_dir'),'lmdb'),readonly=True,lock=False,max_spare_txns=50,readahead=False)
        datalist = np.load(os.path.join(self.dataset_cfg.get('data_dir'),'datalist.npy'),allow_pickle=True)
        if mode =='train':
        ...
        if mode == 'train':
            self.transform = transform.Compose([
                ...
                transform.RandomHorizontalFlip(self.transform_cfg.get("random_flip_prob",0.5))
            ])
        else:
            self.transform = transform.Compose([
                transform.Normalize(
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(buffers=True) as txn:
            frame_names = [file_name+'-'+str(i)+'.jpg' for i in range(1,item['video_len'])]
            frames = [self.jpeg.decode(txn.get(i.encode())) for i in frame_names]
            frames = [cv2.cvtColor(cv2.resize(frame,(128,64)),cv2.COLOR_BGR2RGB) for frame in frames]
        
        video = np.stack(frames)
        video_len = video.shape[0]
        video = np.pad(video,(0,self.video_max_len - video.shape[0])) # (len,64,128,3) [0,255]
